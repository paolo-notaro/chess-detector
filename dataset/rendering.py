...
def setup_scene(chess_set=None):
    ...
    table.data.materials.append(
        bpy.data.materials[random.choice(TABLE_MATERIALS) + "_Table"]
    )
...
def gen_piece_placement_variability():
    ...
    piece_placement_variability = [
        [
            (
                random.uniform(
                    -PIECE_PLACEMENT_VARIABILITY, PIECE_PLACEMENT_VARIABILITY
                ),
                random.uniform(
                    -PIECE_PLACEMENT_VARIABILITY, PIECE_PLACEMENT_VARIABILITY
                ),
...
def arrange_pieces(board: Board, piece_placement_variability=None):
    ...
    for square in SQUARES:
        ...
        if piece is not None:
            ...
            x = (
                A1_POS[0]
                + SQUARE_SIZE * square_file(square)
                + x_variability * SQUARE_SIZE
            )
            y = (
                A1_POS[1]
                + SQUARE_SIZE * square_rank(square)
                + y_variability * SQUARE_SIZE
            )
            z = PIECE_Z_COORD
...
def render_board(
    board, path, board_id, piece_placement_variability=None, suppress_output=True
):
