...
MoveEntry = tuple[str, str, str]
AnalyzedMoves = tuple[dict[str, set[MoveEntry]], int]
...
def select(
    dataset: dict[str, set[MoveEntry]],
    promotions_ratio: float,
    kingside_castle_ratio: float,
    queenside_castle_ratio: float,
    piece_count_ratio: float,
    min_count: int = 0,
    ...
    toadd = random.sample(
        sorted(dataset["p"]),
        min(max(min_count, int(promotions_ratio * len(dataset["p"]))), len(dataset["p"])),
    )
    ...
    toadd = random.sample(
        sorted(dataset["kc"]),
        min(max(min_count, int(kingside_castle_ratio * len(dataset["kc"]))), len(dataset["kc"])),
    )
    selected.update(toadd)
    ...
    toadd = random.sample(
        sorted(dataset["qc"]),
        min(max(min_count, int(queenside_castle_ratio * len(dataset["qc"]))), len(dataset["qc"])),
    )
    selected.update(toadd)
    ...
    for i in range(2, 33):
        toadd = random.sample(
            ...
            min(
                max(min_count, int(piece_count_ratio * len(dataset["d" + str(i)]))),
                len(dataset["d" + str(i)]),
            ),
        )
        selected.update(toadd)
